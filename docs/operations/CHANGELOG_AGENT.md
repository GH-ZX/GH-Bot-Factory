...
- Aligned Admin with public builder identity, including desktop/mobile navigation, cards, forms and quote dialogs. Reused the existing mobile navigation work; separate password-auth changes remain outside this milestone.
- Added structured inquiry briefs, immutable quote scope, one-time customer-owned estimates and recurring-line rejection for new customer-owned quotes; expired/superseded/conflicting acceptance fails closed.
- Added tenant support conversations and purchase-time warranty eligibility/manual decisions, coupons integrated transactionally with checkout, and reviewed/durable announcements. No automatic refund/replacement is implied by warranty approval; unknown broadcasts are never replayed automatically.
- Exposed category/product ordering, HTTPS product images, warranty terms, variant/base-price editing, reseller tier assignment and margin rules. Existing user/order/financial review, audit and alert views remain available.
- Migration `0ce5d2c98e7f` follows `e5f6a8b9c1d2`; six tenant-owned tables plus ordering/scope/sale-term columns. Updated model registry and tenant bundle contract; destination restores cancel queued broadcasts. See ADR-049.
- Workspace verification: `make verify` passed (478 fast, 18 PostgreSQL; migration/no drift and standard static/dependency gates). Mocked browser operations and setup regressions passed. Final isolated milestone verification completed 2026-09-22: canonical `make verify` passed with 474 fast + 19 PostgreSQL tests. Four prior password-auth tests are excluded; the extra PostgreSQL test proves sibling quote acceptance serialization. Exact-source Admin operations and Admin/Mini App browser checks passed.
