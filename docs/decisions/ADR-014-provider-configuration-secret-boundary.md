...
3. Environment secret references accepted through Admin APIs use an environment-style identifier format. Phase 10 additionally permits a write-only credential value: the API immediately stores it through `SecretStorage` (the encrypted local vault in the default self-hosted deployment) and persists only a deterministic reference. Read APIs still never return either the reference or value. See ADR-030.
...
- Browser clients cannot retrieve secret references or previously stored secret values for later probing or social-engineering use.
