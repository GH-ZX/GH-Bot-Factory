...
## Milestone 48: Phase 14.1 Public Configurator & Messaging (2026-09-18)

- **Status:** Delivered & Verified
- **Key Additions:**
  1. Added ADR-040 establishing the public discovery boundary, server-authoritative quote calculation, rate-limited inquiry capture, and domain isolation (preserving apex `gh-store.me` untouched and mounting on `botfac.gh-store.me/build/`).
  2. Created `packages/marketplace/`:
     - `integrations.py`: defines 6 standard integration offerings with setup and recurring fees.
     ...
     - `models.py`: `CustomerInquiry` model with contact details, project notes, configuration snapshot, quote snapshot, status, and IP hash.
  3. Added database migration `a1b2c3d4e5f7_add_customer_inquiries.py` creating table `customer_inquiries` with JSONB types and status/contact indexes.
  4. Implemented public API router `apps/api/v1/public_marketplace.py` exposing `/templates`, `/integrations`, `/estimate`, and `/inquiries`.
  5. Built modern, mobile-first Web App in `apps/build/static/` (`index.html`, `styles.css`, `app.js`) mounted at `/build/` with real-time live price receipt, template search/filtering, add-on selection, and 1-click Telegram deep-link prefill.
  6. Added test suite `tests/test_phase14_1_public_configurator.py` covering all public endpoints, quote calculations, lead persistence, and static page loading.
  7. Rebuilt Docker image `gh-bot-factory:local` and verified live on Compose services (`api`, `worker`, `bot-runtime`).
  8. Canonical gate passed: Ruff clean, 408 fast tests passed, 13 PostgreSQL concurrency tests passed, Alembic no-drift clean at `a1b2c3d4e5f7`.

## Milestone 49: Phase 14.2 Platform Sales Console & Quotes (2026-09-18)

- **Status:** Delivered & Verified
- **Key Additions:**
  1. Added ADR-041 establishing the platform sales boundary, commercial quote immutability, in-memory operator credential gate, and CLI/API operations.
  2. Implemented `CommercialQuote` and `CommercialQuoteLine` models (`packages/marketplace/models.py`) with migration `b2c3d4e5f6a8_add_commercial_quotes.py`.
  3. Implemented platform sales API router `apps/api/v1/platform_sales.py` (`/api/v1/platform/sales/inquiries`, `/quotes`, `/quotes/{id}/accept`) with `require_platform_operator` and non-repudiable `PlatformAuditLog` audit logging.
  4. Built Sales & Leads console in Admin dashboard (`apps/admin/static/`) with in-memory operator token gate (never persisted in browser storage), real-time inquiry inspection, status transitions, and formal quote generator.
  5. Updated `scripts/platformctl.py` with `inquiries`, `inquiry`, `inquiry-status`, `quotes`, `quote`, and `quote-accept` subcommands and auto-resolved LAN bind host.
  6. Added test suite `tests/test_phase14_2_sales_quotes.py` covering platform auth, inquiry lifecycle, quote generation, acceptance immutability, and audit logging.
...
### Post-onboarding advisory — 2026-09-19

User requested further suggestions after `ed9c69a`. Prioritize encrypted/restorable customer handoff, operator-owned deployment of verified fixes, and one complete first-sale trial. Prompt and scope recorded under “Post-Onboarding Priority Advisory” in `docs/prompts/prompt_history.md`; no implementation milestone claimed.
...
## 2026-09-21 — Admin Identity and Tenant Operations

Steps 1/3 implementation: Admin shares `/build/` cream/green identity, adds a readable structured brief and one-time customer-owned quotes, and introduces `packages/operations` with tenant support/warranty cases, percentage coupons and durable announcement deliveries. Routes: `apps/api/v1/admin_operations.py` (`/admin/operations`, `/storefront/support`); UI: `apps/admin/static/operations.js` and `identity.css`. Catalog ordering/warranty terms and checkout discount snapshots extend shared commerce; pricing tier/rule changes are audited. Six new tenant-scoped tables join the portable export contract; imported pending broadcasts are cancelled. Migration `0ce5d2c98e7f`; see ADR-049 and `docs/plans/admin-operations-completion.md`. Existing ledger/refund/provider safeguards remain authoritative. Step 2 real-customer acceptance is pending; steps 4–6 are incomplete. No production qualification or new live deployment is claimed.
